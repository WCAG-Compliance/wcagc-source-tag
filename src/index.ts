...
export function transformSourceTags(code: string, filename: string, options: SourceTagOptions = {}) {
  ...
  return changed ? {
    ...
    map: output.generateMap({hires: true, source: path, includeContent: false})
  } : null;
}

export default function sourceTag(options: SourceTagOptions = {}): Plugin {
  return {
    name: "wcagc-source-tag", enforce: "pre", apply: options.includeInBuild ? undefined : "serve",
    transform(code, id) {
      const filename = id.split("?", 1)[0]!;
      if (!/\.[cm]?[jt]sx$/.test(filename) || options.include && !options.include.test(filename) || options.exclude?.test(filename)) return null;
      return transformSourceTags(code, filename, {...options, root: options.root ?? process.cwd()});
    }
  };
}
